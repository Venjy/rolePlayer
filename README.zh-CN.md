...
- 响应式复盘页只评价和指导真实销售学员，不评价扮演客户的 AI；一次生成会保存同一套评分以及语义一致的中英文整体评价、评分明细、优点、改进项、建议和关键时刻，切换语言只读取已保存结果，不会重新生成。页面还展示会话元数据及可复制/下载的完整记录，底部支持永久删除本次记录，或使用相同角色、场景和难度“再试一次”；格式、引用或角色归因错误的可选关键时刻会被舍弃而不会拖垮主体复盘；旧提示词版本的报告在打开时按需重新生成，失败时会明确指出取数、模型连接、内容校验或保存阶段
