...
- 学员启动页提供可搜索的场景和角色选择、兼容性过滤、Ant Design 简单/中等/困难单选按钮，展示目标、技能、成功标准、角色背景、性格、沟通方式、行为说明、动机和顾虑等备战信息，以及真正完整的 Instructions 预览和 `实际字数/12000` 限制
- 响应式管理控制台，角色/场景独立编辑，兼容关系单独管理，可按需从成功标准生成评分权重，并提供各自独立的 Instructions 预览和由已配置 Qwen 文本模型生成的双语可编辑随机草稿
...
- 角色负责可复用的人物属性和 Qwen 音色；场景负责情境，以及可选的目标、技能、成功标准、评分权重、语气、说话节奏和插话/挑战倾向
...
- 显式暂停/继续会话：暂停前安全结算当前轮次，隐藏语音输入区并停止有效时长累计；继续时从 SQLite 最终历史建立新的 Qwen 连接
- 经过确认后可原地重启：保留同一个会话 ID、角色/场景快照、难度、Instructions 和音色，清空本次文字/音频并将有效时长归零后重新连接
- 切换会话、新建对练、暂停、重启和结束会话会串行执行，并在断开前等待用户/助手的响应级持久化确认；保存失败会明确报错，不会静默丢弃最后一轮
...
- 响应式复盘页只评价和指导真实销售学员，不评价扮演客户的 AI；一次生成会保存同一套评分以及语义一致的中英文整体评价、评分明细、优点、改进项、建议和关键时刻，切换语言只读取已保存结果，不会重新生成。当会话至少有三轮学员发言时，服务端校验后必须保留至少三个引用不同学员消息且有原文依据的关键时刻；不足三轮时保留可用复盘，并显示双语的对话证据不足警告，不会编造卡片。页面还展示会话元数据及可复制/下载的完整记录，底部支持永久删除本次记录，或使用相同角色、场景和难度“再试一次”；旧提示词版本的报告在打开时按需重新生成，失败时会明确指出取数、模型连接、内容校验或保存阶段
...
- 点击开始的长录音同时提供“结束发言”和“取消本次录音”；取消时等待上游音频缓冲区清空确认，绝不会提交被放弃的内容
...
- 有界实时回复恢复：响应开始/流式进展均有 30 秒看门狗，对已消费的 Qwen 事件做结构校验；AI 回复失败、为空或音频畸形时先清理上下文并自动重试一次，重建连接后还能对数据库中尚未回答的最终用户发言重试，且不会重复提交音频

## 持久化状态

新建的目录库和会话库拥有彼此独立的迁移历史，只包含各自领域的表。每一类预设都有独立物理表，角色和场景只引用预设 ID，不再重复保存双语标签。历史合并数据库仍保留迁移 1–20，供 `pnpm database:split` 安全升级并复制旧数据。Schema 迁移只负责结构，业务默认值必须显式初始化。目录 REST API 如下：
...
| `POST` | `/api/conversations/:id/pause` | 持久化暂停状态并停止有效时长累计 |
| `POST` | `/api/conversations/:id/resume` | 重连实时语音前开始一段新的有效计时 |
| `POST` | `/api/conversations/:id/restart` | 保留同一启动快照和 ID，清空文字/音频并重置有效时长 |
...
场景的 `voiceBehavior.interruptFrequency` 只会调整提示词层面的耐心程度、插话和质疑行为。按住说话、长录音和自由对话都保留 Qwen 手动轮次检测（`turn_detection: null`），自由对话由浏览器自动判断人声与持续静音。学员仍可在角色说话时打断，但 Qwen 无法在学员语音尚未提交时自主开始插话。
