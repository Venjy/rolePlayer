...
Create drawers place an Ant Design official-recipe gradient **Random generate / 随机生成** primary button with a leading `✨` emoji between Cancel and Save. The local `ConfigProvider` injects the gradient class, whose `::before` layer uses `inset: -1px` to cover the complete button edge. It uses a purple-to-cyan gradient by default and reverses to a brighter cyan-to-purple gradient on hover. They never close when the operator clicks the background mask, preventing accidental loss of an unsaved generated or manually edited draft. The button sends every current form value as a `currentDraft` exclusion before calling the configured Qwen text model, so a second generation in the same drawer treats the prior result and any operator edits as an existing instance. Persisted persona names/backgrounds and scenario names/descriptions are excluded as well. The returned bilingual draft fills the form and never saves automatically. During generation, the button keeps its label and shows a spinning indicator after the text. Form controls, Save, the close icon, keyboard dismissal, and mask dismissal are disabled, while the explicit Cancel button remains available. Cancel aborts the client request, closes the drawer, and invalidates the request token so even a late response cannot refill the form. Returned preset IDs must come from the current SQLite options, and generated persona gender must match the structured Qwen voice gender. Provider/configuration/timeout/network/invalid-output failures remain in the drawer as localized form feedback.
