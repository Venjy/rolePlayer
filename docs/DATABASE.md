...
The production image fixes both paths under `/app/data`, declares that directory
as a volume, and runs as the non-root `node` user that owns it. During image
construction, the built deployment initializer applies both migration chains
and catalog seeds under `/app/image-data`; the final stage copies the prepared
pair to `/app/data` before declaring the volume. Docker populates a new volume
from those files. Before Fastify starts, the container entrypoint reruns the same
initializer against the mounted paths so existing volumes and bind mounts also
receive later migrations and missing seed rows. The operation is transactional
and idempotent and does not overwrite administrator changes. Back up the mounted
volume—not a running container's writable layer—and stop the container or use a
SQLite-aware backup procedure before copying live database files.
...
└── scenario_id, success_criterion_preset_id, position, weight (nullable)
...
├── status (active or ended), paused_at, ended_at
...
Free-text business fields have explicit English/Chinese columns. Preset-backed fields do not duplicate localized text on personas or scenarios: single selections are foreign-key columns and multi-selections are ordered relation rows. `qwen_voices.voice` and `personas.voice` retain the provider-owned Qwen ID needed by the realtime API; localized display names live only in `qwen_voices.name` / `name_zh_cn`, and `qwen_voices.gender` stores the structured provider voice gender used to validate generated personas. Scenario voice behavior uses nullable `interrupt_frequency` and `speaking_pace` columns. Success criteria always use ordered `scenario_success_criteria` rows; a nullable `weight` distinguishes criteria without a numerical rubric from fully weighted scoring rows.
...
`sessions` stores the exact compiled Instructions, voice, difficulty, locale, and lifecycle timestamps needed to rebuild a realtime connection. Its immutable catalog snapshot is normalized into `persona_snapshots`, `scenario_snapshots`, `scenario_scoring_criteria`, and `scenario_personas`. Catalog edits therefore affect only future conversations without putting whole business objects into JSON columns.

Active practice time is accumulated in segments. `active_started_at` marks the current running segment; pausing adds that segment to `active_duration_ms`, sets `paused_at`, and clears `active_started_at`. Resuming clears `paused_at` and starts a new segment. The summary/detail API adds the current running segment at read time without mutating SQLite. Feedback uses the final accumulated value, so paused time is not included. Closing the last realtime socket pauses idempotently and configuring a replacement socket resumes idempotently, covering abrupt browser disconnects as well as explicit controls.

Restart is an in-place reset for an unended session: one transaction deletes `messages` (cascading `message_audio`) and any feedback row, resets active time and lifecycle timestamps, and retains the same session ID plus every immutable snapshot/configuration row. This differs from feedback **Try again**, which creates a new session from current catalog data. Ended sessions cannot be paused, resumed, or restarted.
...
Migrations are append-only structural changes. The catalog chain currently has nine entries: migration 4 moves both historical discriminator tables into nine independent domain tables, migration 5 replaces duplicated catalog text with preset foreign keys/ordered relations, migration 6 creates the bilingual Qwen voice directory, migration 7 moves tone/pace/interjection configuration from personas to scenarios, migration 8 records structured Qwen voice gender, and migration 9 makes success-criterion weights nullable so criteria and scoring can be configured independently. The conversation chain has nine entries: migration 4 removes the redundant `conversation_` prefix from business table and index names, migration 5 adds finalized-message audio, migration 6 moves the resolved voice-behavior values from persona snapshots to scenario snapshots, migration 7 adds terminal session state plus normalized feedback tables, migration 8 adds paired Simplified Chinese feedback text columns without duplicating scores or references, and migration 9 adds pause plus active-duration accounting. Neither file contains the other domain's business tables.

The historical combined database uses migrations 1–20:
...
- migration 19: add durable pause state and segmented active-practice duration accounting.
- migration 20: make success-criterion weights nullable so a scenario may keep completion criteria without a numerical rubric.

During migration 9/19, historical ended sessions receive the best available wall-clock duration because earlier schemas could not record pauses. Historical active sessions are marked paused at their last `updated_at` with zero accumulated active time; this avoids counting unknown disconnected time and requires an explicit resume before new realtime writes.
...
To migrate both deployment databases and initialize catalog data in one command:

```bash
pnpm database:init:prod
```
...
For a consistent local backup, stop the server and copy both `.sqlite` files. Under the current `DELETE` journal mode, a clean shutdown leaves no WAL/SHM sidecars. Do not discard a leftover `-journal` file after an abnormal shutdown before SQLite has had an opportunity to recover. The Docker image already contains a migrated and seeded database pair, while its entrypoint runs `database:init:prod` against the mounted persistent directory before every Fastify start. Keep the volume mounted so runtime changes survive image replacement.
