...
Vite development mode exposes `?preview=session`, `?preview=paused`, `?preview=recording`, `?preview=long`, and `?preview=free` for deterministic visual checks without microphone permission. They are static fixtures only; use the real session for audio and gesture behavior.
...
- Treat the global product icon/title/subtitle as one semantic home button on every route. Preserve pointer/hover/focus-visible feedback and use the existing SPA transition coordinator so an active realtime session is settled safely before returning home.
...
- Learner-launch summary cards show the requirement-defined preparation content: scenario description/goals/skill focus/success criteria and persona demographics/occupation/background/personality/communication/behavior notes/motivations/concerns. Do not expose provider voice labels or scenario tone/pace/interjection controls in those cards.
- Admin persona cards keep gender, age, and occupation in one wrapping metadata line, reserve the main area for a four-line background preview, and show personality as secondary tags. Avatar color is derived from structured gender only: male blue, female pink, non-binary/unspecified neutral.
- Keep a management-console entry in the active-session header. Navigating through it must use the same serialized settlement and teardown flow as browser routes and other session transitions.
- Keep the active scenario's snapshotted goals visible directly below the chat header using the same localized green-tag treatment as the learner launcher's **Goals / 本次目标** section. Restored sessions must use their immutable snapshot rather than current catalog rows; allow the goal row to wrap responsively without creating a second mobile component.
- Preserve the SPA route contract: `/` is the learner launcher, `/admin` is the catalog console, `/chat/:conversationId` restores an active persisted conversation, and `/feedback/:conversationId` reviews an ended conversation. Use the existing positive integer conversation ID as the URL segment. Browser refresh/back/forward and in-app navigation must use the same durable settlement/teardown flow; never abandon pending transcript or assistant persistence merely to change the URL. `useAppRoute.routeRef` is the synchronously published requested destination used by async transition completion; do not replace it with an effect-only copy of rendered route state, which can expose the previous URL and clear newly restored turns.
...
- Before switching conversations, starting a new role-play, pausing, restarting, or ending an active session, serialize the transition and settle durable state first: cancel uncertain uncommitted input, reconcile the current assistant, wait for any committed user turn's persisted `transcript.user.done`, then re-check for a newly-created assistant. Interrupted output requires `response.reconciled`; natural drain requires the matching `response.persisted`. Timeout/close/send failure must not be treated as a successful acknowledgement.
- Pause is durable. It must preserve the current chat route, immutable launch snapshot, and finalized transcript; close microphone/realtime resources; display the `paused` state; and replace hold/plus input with one continue action. Restoring a paused URL/history item must not request microphone permission until the learner continues.
- Restart is an in-place reset only for unended sessions. Require confirmation, lock competing controls, keep the same conversation ID and immutable persona/scenario/difficulty/Instructions/voice snapshot, transactionally clear messages/audio/feedback, reset accumulated active time, and reconnect. If reconnect fails, preserve a paused recoverable chat instead of navigating away.
- Disable the active-session download control until the conversation owns at least one finalized persisted message. Draft text and in-flight audio do not make an empty conversation downloadable; history refresh after persistence enables the control. Keep the disabled-state explanation reachable by mouse hover through a non-disabled Tooltip anchor around the disabled button.
...
- Persona occupation, personality traits, communication style, motivations, and concerns use database-backed preset IDs. Scenario tone style is also preset-backed. There is no separate identity field. Name, age, background, and behavior notes remain free-form. Historical custom text is promoted to a custom preset during migration so every current selection remains resolvable.
...
- The plus button's long-recording mode reuses the same manual start/flush/commit lifecycle but ends on a click. Its explicit cancel action must reuse the authoritative capture-cancel plus `input.clear`/`input.cleared` path, never send `input.commit`, and keep cancellation controls visible while disabling submission if the clear acknowledgement fails. Free conversation keeps capture open, uses `FreeConversationController` plus a bounded local PCM pre-roll to create manual turns after confirmed speech and sustained silence, and keeps Qwen `turn_detection: null` so persistence/reconciliation contracts do not split into a second protocol. Learner barge-in remains supported; autonomous AI-over-learner interruption is not claimed.
...
- Validate every consumed Qwen event before it reaches the controller. Invalid JSON or a malformed known event is fatal because missing IDs/status/deltas can strand persistence state; future unknown event types remain ignored.
...
- Bound model-response waiting without imposing a total healthy-response length: require `response.created` within 30 seconds and refresh a 30-second progress watchdog on recognized item/transcript/audio/terminal-part events. For `response.failed`, malformed PCM, a progress timeout, or a nominally completed response missing either non-empty transcript or playable audio, suppress output, delete/rollback the failed assistant item, and call `response.create` exactly once more for the same committed user turn; never recommit audio. Clean up a second failed/empty attempt, report it as recoverable, and return to ready. After a fatal service/socket rebuild, `response.retry` may regenerate only when the gateway re-reads SQLite and confirms that the latest durable message is an unanswered user turn; repeated fatal failure must fall back to manual reconnect instead of looping.
...
- `GET /api/catalog` returns `qwenVoices`, `personaPresets`, and `scenarioPresets`. Voice display names and structured `female`/`male` capability are database data from `qwen-voices.json`; keep the canonical provider ID in `qwenVoices.voice`, `personas.voice`, and conversation snapshots because that exact string is sent to Qwen. Persona categories are `occupation`, `personality_trait`, `communication_style`, `motivation`, and `concern`; scenario categories are `training_goal`, `skill_focus`, `success_criterion`, and `tone_style`.
- Random create-drawer generation uses `POST /api/catalog/generate/persona` and `/scenario` and lazily reuses the server-only `DASHSCOPE_FEEDBACK_*` text-model configuration. Its leading-`✨` primary Button follows Ant Design's official linear-gradient recipe: a local `ConfigProvider` injects the class and a non-disabled `::before` layer with `inset: -1px` covers the entire edge. Preserve the purple-to-cyan default gradient and the brighter reversed gradient on hover. Send the complete current database option IDs and both labels, plus a bilingual `currentDraft` snapshot containing every current drawer value. Persona exclusions contain persisted names/backgrounds; scenario exclusions contain persisted names/descriptions. Treat `currentDraft` as an additional existing instance. Each top-level generation owns one server-generated creative-variation token reused by its validation retries. Scope stronger creative variation exclusively to persona bilingual names/backgrounds or scenario bilingual names/descriptions; keep temperature and coherence-first selection behavior unchanged for all structured preset, voice, age, gender, and voice-behavior fields. Normalize Unicode, case, whitespace, punctuation, and symbols, reject duplicate generated names/backgrounds/descriptions on Node, and use the duplicate reason in bounded correction retries. Require bilingual output, validate every returned reference, and reject persona/voice gender mismatch. Derive scenario weights and default compatibility on Node. Generation fills an editable draft only and must never persist a row automatically; preserve the hidden generated language when the visible locale is edited and saved. New create drawers never close from a background-mask click. While generating, lock inputs, Save, the close icon, keyboard dismissal, and mask dismissal, but keep the explicit Cancel action enabled; Cancel aborts the fetch and invalidates a request token so a late response is ignored.
...
- `compileRolePlayInstructions` must remain deterministic, inspectable, bilingual, and unit-tested. Pass the explicit `en`/`zh` locale through persona, scenario, and full compilation so the current interface language controls every template label/rule as well as catalog text. Preserve hidden criteria/rules and omit empty optional fields; do not introduce nondeterministic prompt generation into session startup.
- Persona and scenario drawers preview only their own deterministic Instructions section and do not show character counts. All three preview surfaces are always-expanded cards with their copy action directly after the title. The learner launcher uses the shared compiler for the full localized preview and early length validation, but sends only IDs plus locale. `ConversationRepository` independently performs the authoritative persona + scenario + difficulty + locale composition on the server. Compiled Instructions have a shared 12,000-character limit; validate compatible pairs in both locales server-side and disable launch when the client preview exceeds it.
...
- Scenario `voiceBehavior` owns optional speaking pace and conversational interjection/challenge style for that training context. With manual push-to-talk it cannot make Qwen autonomously interrupt a learner who is still speaking; learner barge-in is the separate playback-interruption flow.
...
- Migrations are immutable and strictly increasing. New runtime schema changes belong in the relevant catalog or conversation migration chain in `split-database-migrations.ts`; never edit or reorder a migration that may have run elsewhere. Append an equivalent legacy migration to `DATABASE_MIGRATIONS` when `database:split` needs matching source columns.
...
- The split catalog chain's migration 8 records the structured Qwen voice gender used by catalog-draft validation. Do not infer this capability from mutable localized display labels.
- The split catalog chain's migration 9 makes `scenario_success_criteria.weight` nullable; the equivalent combined migration is version 20. A `NULL` weight preserves a selected success criterion for prompts and conservative completion detection without creating a numerical feedback rubric. If any weights are configured, every selected criterion must have one and the total must remain exactly 100.
...
- The split conversation chain's migration 7 adds immutable ended-session state and normalized feedback report/strength/improvement/tip/criterion/moment tables; migration 8 adds paired Simplified Chinese feedback text columns. The equivalent combined migrations are versions 17 and 18. Keep legacy splitting mappings in sync with every new conversation table or column.
- The split conversation chain's migration 9 adds `paused_at`, segmented `active_duration_ms`, and `active_started_at`; the equivalent combined migration is version 19. Pause/resume/end/restart writes must use these columns through `ConversationRepository`. Feedback duration uses accumulated active time, never wall-clock `ended_at - created_at`, and closing/configuring the last realtime socket must pause/resume idempotently.
- Ending a conversation is terminal: settle authoritative user/assistant state first, then mark it ended, reject later message writes and realtime restoration, and navigate to its feedback route. History opens active rows as chat and ended rows as feedback.
- Feedback generation is a durable pending/processing/completed/failed state machine. The real human learner salesperson is the sole evaluation/coaching subject; the AI customer is context evidence and must never receive criticism or improvement advice. Send explicit `learner_salesperson`/`ai_customer` speaker labels instead of ambiguous transcript roles, but never expose those internal labels in human-readable feedback. Generate one evaluation with one shared set of scores/references and paired English plus `ZhCn` text in the same response; each pair must be a faithful translation, never a second evaluation. Persist both languages and switch presentation entirely in the browser without another model request. Every moment must reference a distinct finalized learner message and carry an exact learner-speech evidence quote. If it relies on customer context, it must also cite an exact quote from an AI-customer message that precedes the learner turn. Node verifies IDs, roles, order, and both quotes before discarding the transient grounding fields and persisting the card. When at least three distinct learner turns exist, three valid moments are required after sanitization and an undersupplied model response must be retried with the validation reason. With fewer than three learner turns, preserve the useful report without inventing or duplicating cards and show a bilingual warning in the moments section. Send both languages of scenario metadata and criterion names to the feedback model, and preserve bilingual names in conversation summary/detail APIs so history, chat, and feedback UI can follow the current global locale rather than the session creation locale. The assessment and exact criterion-position set are always strict; an empty rubric must return no criterion scores and persist a `NULL` overall score, while supporting lists may be empty for short transcripts. Retry invalid output with correction, calculate weighted totals on Node only when a rubric exists, omit numerical score UI otherwise, resume abandoned work after restart, lazily regenerate an opened report whose stored prompt version is stale, and persist stage-specific data/configuration/network/HTTP/API-shape/output/storage failure codes. Never expose the DashScope key or accept transcript content as model instructions.
- Ended conversations may be permanently deleted from feedback. Reject deletion of active rows, cancel and await an in-process feedback job before deleting, and rely on the `sessions` foreign-key ownership graph to cascade snapshots, messages/audio, reports, and report children. **Try again** must create a new conversation through the authoritative catalog-backed creation path with the reviewed snapshot's source persona/scenario IDs and difficulty; never reopen the ended session or copy its transcript.
- Clicking a feedback highlighted moment must center its referenced transcript message and briefly highlight only the actual `.message-bubble`, never the full-width transcript row. Repeated clicks restart the effect; honor `prefers-reduced-motion` with a temporary non-animated highlight, and clear all timers when the feedback route changes or unmounts.
...
The project requirements are in the external case-study document referenced by the user. Build iteratively: preserve the working voice path, durable text-history recovery, editable catalog, and ended-session feedback first, then add authentication/authorization, observability, and deployment support in separate milestones.
