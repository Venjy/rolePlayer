...
Vite development mode exposes `?preview=session`, `?preview=recording`, `?preview=long`, and `?preview=free` for deterministic visual checks without microphone permission. They are static fixtures only; use the real session for audio and gesture behavior.
