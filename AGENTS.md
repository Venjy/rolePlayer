...
- The split conversation chain's migration 7 adds immutable ended-session state and normalized feedback report/strength/improvement/tip/criterion/moment tables. The equivalent combined migration is version 17. Keep legacy splitting mappings in sync with every new conversation table.
...
- Feedback generation is a durable pending/processing/completed/failed state machine. Project scenario metadata and criterion names into the stored conversation locale before sending them to the feedback model, but preserve both `name` and `nameZhCn` in the snapshot/API so UI locale switching can relabel score breakdowns without regenerating the report. The assessment and exact criterion-position set are the strict core; supporting lists may be empty for short transcripts, and optional moments must be independently sanitized, capped by distinct finalized user-message IDs, and never invalidate an otherwise usable core report. Retry invalid core output with correction, calculate weighted totals on Node, resume abandoned work after restart, and persist stage-specific data/configuration/network/HTTP/API-shape/output/storage failure codes. Never expose the DashScope key or accept transcript content as model instructions.
