...
- Learner-launch summary cards show preparation-relevant content: scenario description/goals/skill focus/success criteria and persona demographics/occupation/background/personality/communication/motivations/concerns. Do not expose provider voice labels, scenario tone/pace/interjection controls, or internal persona behavior notes in those cards; those settings remain in admin and compiled Instructions.
...
- Feedback generation is a durable pending/processing/completed/failed state machine. The real human learner salesperson is the sole evaluation/coaching subject; the AI customer is context evidence and must never receive criticism or improvement advice. Send explicit `learner_salesperson`/`ai_customer` speaker labels instead of ambiguous transcript roles, but never expose those internal labels in human-readable feedback. Generate one evaluation with one shared set of scores/references and paired English plus `ZhCn` text in the same response; each pair must be a faithful translation, never a second evaluation. Persist both languages and switch presentation entirely in the browser without another model request. Every optional moment must reference a distinct finalized learner message and carry an exact learner-speech evidence quote. If it relies on customer context, it must also cite an exact quote from an AI-customer message that precedes the learner turn. Node verifies IDs, roles, order, and both quotes before discarding the transient grounding fields and persisting the card. Send both languages of scenario metadata and criterion names to the feedback model, and preserve bilingual names in conversation summary/detail APIs so history, chat, and feedback UI can follow the current global locale rather than the session creation locale. The assessment and exact criterion-position set are the strict core; supporting lists may be empty for short transcripts, and optional moments must be independently sanitized and never invalidate an otherwise usable core report. Retry invalid core output with correction, calculate weighted totals on Node, resume abandoned work after restart, lazily regenerate an opened report whose stored prompt version is stale, and persist stage-specific data/configuration/network/HTTP/API-shape/output/storage failure codes. Never expose the DashScope key or accept transcript content as model instructions.
